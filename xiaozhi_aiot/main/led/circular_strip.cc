...
void CircularStrip::RainbowChase(int interval_ms) {
    ...
    // 初始化颜色数组
    for (int i = 0; i < max_leds_; i++) {
        colors_[i] = rainbow[i % 8];
    }
    
    // 创建旋转彩虹效果的定时器任务
    StartStripTask(interval_ms, [this, rainbow]() {
        // 保存第一个颜色
        StripColor first = colors_[0];
        
        // 将所有颜色向前移动一位
        for (int i = 0; i < max_leds_ - 1; i++) {
            colors_[i] = colors_[i + 1];
        }
        
        // 将第一个颜色移到末尾
        colors_[max_leds_ - 1] = first;
...
void CircularStrip::StopEffects() {
    std::lock_guard<std::mutex> lock(mutex_);
    if (strip_timer_ != nullptr) {
        esp_timer_stop(strip_timer_);
        ESP_LOGI(TAG, "Stopped strip timer effects");
    }
